...
public class Enemy : MonoBehaviour
{
    ...
    public float shootAccuaracy = 0.1f;
                ...
                if (_enemyToPlayer.magnitude <= detectionRange)
                {
                    if (Physics.Raycast(shootOrigin.position, _enemyToPlayer, out RaycastHit _hit, detectionRange))
                    {
                        if (_hit.collider.CompareTag("Player"))
                        {
                            ...
                            SetRotation(new Vector3(_enemyToPlayer.x, 0f, _enemyToPlayer.z));

                            if (isPatrolRoutineRunning)
                            {
                                isPatrolRoutineRunning = false;
                                StopCoroutine(StartPartol());
                            }
    ...
    private void Patrol()
    {
        if (!isPatrolRoutineRunning)
        {
            StartCoroutine(StartPartol());
        }

        Move(transform.forward, patrolSpeed);
    }

    private IEnumerator StartPartol()
    {
        isPatrolRoutineRunning = true;
        Vector2 _randomPatrolDirection = Random.insideUnitCircle.normalized;
        SetRotation(new Vector3(_randomPatrolDirection.x, 0f, _randomPatrolDirection.y));
    ...
    private void Move(Vector3 _direction, float _speed)
    {
        ...
        ServerSend.EnemyPosition(this, false);
    }
    ...
    private void Shoot(Vector3 _shootDirection)
    {
        ...
        ServerSend.EnemyShot(this, _shootDirection);

        if (Physics.Raycast(shootOrigin.position, _shootDirection, out RaycastHit _hit, shootRange))
        {
            if (_hit.collider.CompareTag("Player"))
            {
                if (Random.value <= shootAccuaracy)
                {
                    _hit.collider.GetComponent<Player>().TakeDamage(50f);
                    ServerSend.PlayerKilled("Bot", _hit.collider.GetComponent<Player>().username);
                }
            }
        }
    ...
    private void SetRotation(Vector3 _newRotation)
    {
        transform.forward = _newRotation;
